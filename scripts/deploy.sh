...
# a failed build must stop here — the first deploy piped it through tail, lost the exit code, and started pm2 on nothing
npm run build > build.log 2>&1 || { tail -25 build.log; echo "BUILD FAILED"; exit 1; }
tail -3 build.log
